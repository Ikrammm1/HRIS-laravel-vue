...
export default class AuthService extends BaseService {
    ...
    login(payload) {
        ...
        // Ensure device_name is present for Sanctum token generation
        const loginPayload = {
            device_name: 'web-browser',
            ...payload
        };
        return this.get("/sanctum/csrf-cookie").then(() => {
            // Use API token endpoint for SPA (returns plain text token)
            return this.post("/api/auth/login", loginPayload)
        }).finally(() => {
    ...
    logout() {
        ...
        // Revoke token via API logout endpoint
