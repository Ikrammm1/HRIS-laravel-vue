...
export default {
  ...
  actions: {
    async fetchAll({ commit }) {
      ...
      const res = await axios.get('/api/setup/work-schedules/list');
      commit('SET_DATAS', res.data.data);
      commit('SET_LOADING', false);
    },
    async process({ commit }, { formData, id }) {
      if (id) {
        const res = await axios.put(`/api/setup/work-schedules/update/${id}`, formData);
        commit('UPDATE_DATA', res.data.data);
      } else {
        const res = await axios.post('/api/setup/work-schedules/add', formData);
        commit('ADD_DATA', res.data.data);
      }
    },
    async remove({ commit }, id) {
      await axios.delete(`/api/setup/work-schedules/delete/${id}`);
